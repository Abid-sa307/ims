...
import { List, Search, Save, AlertTriangle } from 'lucide-react';
import { cn } from '@/lib/utils';
import { toast } from 'sonner';

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Stock Management', href: '#' },
    { title: 'Missing Entry', href: '/stock/missing-entry' },
];
...
export default function MissingEntry({
    locations = [],
    warehouses = [],
    categories = [],
    items = [],
    ...
    const handleAddItem = () => {
        ...
        if (data.items.some(i => i.item_id === selectedItem.id)) {
            toast.error('Item already added to missing list');
            return;
        }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post('/stock/missing-entry', {
            onSuccess: () => {
                reset();
                toast.success('Missing entries recorded successfully');
            }
        });
    };
            ...
            <div className="flex flex-col gap-4 p-4 lg:p-6 bg-gray-50/50 min-h-screen">
                <div className="flex items-center justify-between border-b pb-4 mb-2">
                    <div className="flex items-center gap-2">
                        ...
                        <h1 className="text-xl font-bold text-[#162a5b]">Missing Entry</h1>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                <Card className="border-none shadow-sm bg-white p-6">
                    <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-6 items-end">
                        ...
                        <div className="lg:col-span-1 flex gap-2 items-end">
                            <div className="flex-1 space-y-1.5">
                                <Label className="text-gray-600 text-sm">Global Remarks</Label>
                                <Input 
                            ...
                            <Button onClick={handleAddItem} className="bg-[#4caf50] hover:bg-[#43a047] text-white uppercase font-bold text-xs h-10 px-6">Add</Button>
                        </div>
                    </div>
                        ...
                        <Table>
                            <TableHeader className="bg-gray-50 border-b">
                                <TableRow>
                                    ...
                                    <TableHead className="font-bold text-gray-700 text-center">UOM</TableHead>
                                    <TableHead className="font-bold text-gray-700 text-center">Missing Qty</TableHead>
                            ...
                            <TableBody>
                                {data.items.length > 0 ? data.items.map((item, index) => (
                                    <TableRow key={item.item_id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <TableCell className="text-center">{item.uom_name || '-'}</TableCell>
                                        <TableCell className="flex justify-center">
                                            <Input 
                                                type="number" 
                                                className="h-8 w-24 border-gray-200 text-center" 
                                                value={item.missing_quantity}
                                                onChange={e => handleItemDataChange(index, 'missing_quantity', e.target.value)}
                                            />
                                        </TableCell>
                                        <TableCell>
                                            <Input 
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={7} className="h-32 text-center text-gray-400">
                                            Add missing items using the search fields above.
                                        </TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                    ...
                    <div className="mt-6">
                        ...
                        >
                            {processing ? 'Saving...' : 'Save Missing Entry'}
                        </Button>
                    </div>
                </Card>
                ...
                <div className="mt-8">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-lg font-bold text-[#162a5b]">Recent Missing Entries</h2>
                        </div>
                        <span className="text-xs text-gray-500 uppercase font-semibold">Audit History</span>
                    </div>
                        ...
                        <Table>
                            <TableHeader className="bg-[#162a5b] hover:bg-[#162a5b]">
                                <TableRow>
                                    ...
                                    <TableHead className="text-white font-bold h-10 py-0 text-center">Missing Qty</TableHead>
                                ...
                                {recentEntries && recentEntries.length > 0 ? (
                                    recentEntries.map((entry) => (
                                        <TableRow key={entry.id} className="hover:bg-gray-50/50 transition-colors border-b last:border-0 border-gray-100 h-10 py-0">
                                            ...
                                            <TableCell className="py-2 text-[11px] font-semibold text-[#162a5b]">{entry.item?.item_name || 'item deleted'}</TableCell>
                                            <TableCell className="py-2 text-[11px] font-bold text-orange-600 text-center">-{entry.missing_quantity}</TableCell>
