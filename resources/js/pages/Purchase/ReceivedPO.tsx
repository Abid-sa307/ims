...
import { Box, PackageCheck, Clock, CheckCircle2, Filter, XCircle } from 'lucide-react';
...
export default function ReceivedPO({ purchaseOrders, filters }: Props) {
    const [dateFrom, setDateFrom] = useState(filters.date_from || '');
    const [dateTo, setDateTo] = useState(filters.date_to || '');

    const handleFilter = () => {
        router.get('/purchase/received-po', { date_from: dateFrom, date_to: dateTo }, { preserveState: true });
    };

    const clearFilter = () => {
        setDateFrom('');
        setDateTo('');
        router.get('/purchase/received-po', {}, { preserveState: true });
    };
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Received PO from Supplier" />
            <div className="flex h-full flex-col p-6 bg-gray-50/50">
                <div className="flex flex-col md:flex-row items-center justify-between border-b pb-6 mb-8 border-t-2 border-t-green-500 bg-white p-6 shadow-sm rounded-xl gap-4">
                    <div className="flex items-center gap-4">
                        <div className="size-12 rounded-full bg-green-50 flex items-center justify-center text-green-600">
                            <PackageCheck className="size-6" />
                        </div>
                        <div>
                            <h1 className="text-lg font-bold text-gray-900">Purchase Reception</h1>
                            <p className="text-sm text-gray-500">History and reception of sent orders.</p>
                        </div>
                    </div>
                    ...
                    {purchaseOrders.length > 0 ? (
                        purchaseOrders.map((po) => (
                            <div key={po.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                                ...
                                {po.status === 'sent' ? (
                                    <Button 
                                        className="w-full bg-[#5cb85c] hover:bg-[#4cae4c] h-9 text-xs font-bold text-white shadow-lg shadow-green-100 flex items-center justify-center gap-2"
                                        onClick={() => router.post(`/purchase/receive-po/${po.id}`)}
                                    >
                                        <Box className="size-4" /> MARK AS RECEIVED
                                    </Button>
                                ) : (
                                    <div className="w-full bg-green-50 text-green-600 h-9 text-[10px] font-black uppercase rounded-lg border border-green-100 flex items-center justify-center gap-2">
                                        <CheckCircle2 className="size-4" /> Goods Received
                                    </div>
                                )}
                            </div>
                        ))
                        ...
                        <div className="col-span-full py-20 text-center text-gray-400">
                            <PackageCheck className="size-12 mx-auto mb-4 opacity-10" />
