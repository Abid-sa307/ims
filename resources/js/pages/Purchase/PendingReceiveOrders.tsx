...
import { Calendar, Filter, Plus, Menu, Search, AlignJustify, PackageCheck, Clock } from 'lucide-react';
import { useState } from 'react';

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Purchase', href: '#' },
    { title: 'Pending Receive Orders', href: '/purchase/pending-receive-orders' },
];

export default function PendingReceiveOrders() {
    const { purchaseOrders, locations, suppliers, filters } = usePage().props as any;
    ...
    const handleFilter = () => {
        router.get(route('purchase.pending-receive-orders'), filterState, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const handleReceive = (id: number) => {
        if (confirm('Are you sure you want to receive this order? This will update your stock.')) {
            router.post(route('purchase.receive-po', id));
        }
    };
                                ...
                                {purchaseOrders.length > 0 ? purchaseOrders.map((po: any, index: number) => (
                                    <tr key={po.id} className="hover:bg-blue-50/20 transition-colors group">
                                        <td className="px-4 py-3 text-center border-r border-gray-50/50">
                                            <Button 
                                                variant="outline" 
                                                size="icon" 
                                                className="h-7 w-7 text-blue-600 border-blue-200 hover:bg-blue-600 hover:text-white transition-all shadow-sm active:scale-95"
                                    ...
                                    <tr>
                                        <td colSpan={12} className="py-20 text-center text-gray-300 font-bold uppercase tracking-[0.2em] italic bg-white">
                                            No Pending Receive Orders Available
                                        </td>
                                    </tr>
                                )}
                            </tbody>
