...
import { Calendar, Filter, Plus, Menu, Search, AlignJustify, Send, CheckCircle2, FileText } from 'lucide-react';
...
export default function SendPO({ purchaseOrders, locations, suppliers, filters }: Props) {
    ...
    const handleFilter = () => {
        router.get('/purchase/send-po', filterState, {
            preserveState: true,
            replace: true
        });
    };
    ...
    const handleSend = (id: number) => {
        router.get(`/purchase/orders/${id}/transmit`);
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Send PO to Supplier" />
            <div className="flex h-full flex-col p-4 sm:p-6 lg:p-8 bg-gray-50/50 overflow-y-auto">

                <div className="flex items-center justify-between border-b pb-4 mb-6">
                    <h1 className="text-xl font-bold tracking-tight text-[#162a5b]">Send PO to Supplier</h1>
                    ...
                    <div className="p-4 border-b border-gray-100 bg-white">
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                            ...
                            <div className="md:col-span-4 lg:col-span-3 space-y-2">
                                <Label className="text-xs text-muted-foreground font-semibold uppercase tracking-wider">Location / Franchise</Label>
                                <select 
                                ...
                                >
                                    <option value="">All Locations ({locations?.length || 0})</option>
                        ...
                        <table className="w-full text-xs text-left align-middle border-collapse">
                            <thead>
                                <tr className="bg-white border-b border-gray-100">
                                    <th className="px-4 py-3.5 font-bold text-[#162a5b] uppercase text-[10px] tracking-widest border-r border-gray-50">Send</th>
                                    ...
                                    <th className="px-4 py-3.5 font-bold text-[#162a5b] uppercase text-[10px] tracking-widest border-r border-gray-50 text-center text-blue-600">Inv No</th>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {purchaseOrders.length > 0 ? purchaseOrders.map((po: any, index: number) => (
                                    <tr key={po.id} className="hover:bg-indigo-50/20 transition-colors group">
                                        <td className="px-4 py-3 text-center border-r border-gray-50/50">
                                            {po.status === 'approved' ? (
                                                <Button 
                                                    variant="outline" 
                                                    size="icon" 
                                                    className="h-7 w-7 text-[#162a5b] border-[#162a5b]/20 hover:bg-[#162a5b] hover:text-white transition-all shadow-sm active:scale-95"
                                                ...
                                                <div className="h-7 w-7 rounded-md bg-green-50 flex items-center justify-center text-green-600 mx-auto border border-green-100 shadow-sm" title="Already Sent">
                                                    <CheckCircle2 className="h-4 w-4" />
                                                </div>
                                            )}
                                        </td>
                                        <td className="px-4 py-3 text-gray-500 border-r border-gray-50/50 text-center font-medium tabular-nums">{index + 1}</td>
                                        <td className="px-4 py-3 font-bold text-[#162a5b] border-r border-gray-50/50 text-center uppercase text-[10px] tracking-tighter">
                                            {po.reference_bill_no || '---'}
                                        </td>
                                        ...
                                        <td className="px-4 py-3 text-right font-black text-[#162a5b] bg-indigo-50/20 tabular-nums">
                                    ...
                                    <tr>
                                        <td colSpan={12} className="py-20 text-center text-gray-300 font-bold uppercase tracking-[0.2em] italic bg-white">
                                            No Transmission Orders Available
                                        </td>
                                    </tr>
                                )}
                            </tbody>
