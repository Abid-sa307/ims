...
import { Calendar, MessageSquare, PlusCircle, Trash2, List, Zap, Save } from 'lucide-react';
import { useEffect, useMemo } from 'react';
import { cn } from '@/lib/utils';

const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Purchase', href: '#' },
    { title: 'Purchase Summary', href: '/purchase/summary' },
...
export default function EditPO({ purchaseOrder, locations, suppliers, items }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        location_id: purchaseOrder.location_id || '',
        supplier_id: purchaseOrder.supplier_id || '',
        reference_bill_no: purchaseOrder.reference_bill_no || '',
        reference_challan_no: purchaseOrder.reference_challan_no || '',
        po_date: purchaseOrder.po_date || '',
        exp_order_date: purchaseOrder.exp_order_date || '',
        inv_date: purchaseOrder.inv_date || '',
        discount_amount: purchaseOrder.discount_amount || 0,
        total_tax_amount: purchaseOrder.total_tax_amount || 0,
        cgst_amount: purchaseOrder.cgst_amount || 0,
        sgst_amount: purchaseOrder.sgst_amount || 0,
        igst_amount: purchaseOrder.igst_amount || 0,
        utgst_amount: purchaseOrder.utgst_amount || 0,
        additional_charges: purchaseOrder.additional_charges || 0,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit PO: ${purchaseOrder.order_number}`} />
            <form onSubmit={submit} className="flex h-full flex-col p-4 sm:p-6 lg:p-8 overflow-y-auto bg-gray-50/50">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b pb-4 mb-6 gap-4">
                    <div>
                        <h1 className="text-xl font-bold tracking-tight text-[#162a5b]">Edit Purchase Order</h1>
                        <p className="text-[10px] text-gray-500 font-medium uppercase tracking-widest mt-0.5">Modifying Order: {purchaseOrder.order_number}</p>
                    </div>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-end gap-6 mt-6 pt-6 border-t border-gray-100">
                    ...
                    <Button type="submit" disabled={processing} className="bg-indigo-600 hover:bg-indigo-700 text-white font-black italic uppercase px-12 h-11 text-xs rounded-xl shadow-lg transform active:scale-95 transition-transform flex items-center gap-2">
                        {processing ? 'SAVING...' : <><Save className="size-4" /> UPDATE PURCHASE ORDER</>}
                    </Button>
                </div>
            </form>
        </AppLayout>
